            ...
            <div class="p-10">
                ...
                <div class="flex flex-col justify-center">
                    <form class="w-full p-6 space-y-6 sm:px-10 sm:space-y-8 bg-green-50 border border-green-500"
                        method="post" action="{{ route('alimento.update', ['alimento' => $alimento]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="flex flex-wrap">
                            ...
                            <input id="data" type="date" class="focus:ring focus:ring-green-200 form-input w-full" name="data"
                                value="{{ $alimento->data }}" required>
                            {{ $errors->has('data') ? $errors->first('data') : '' }}

                        </div>

                        <div class="flex flex-wrap">
                            <label for="descricao" class="block text-black text-sm font-bold mb-2 sm:mb-4">
                                {{ __('Descrição') }}:
                            </label>

                            <input id="descricao" type="text" class="focus:ring focus:ring-green-200 form-input w-full" name="descricao"
                                value="{{ $alimento->descricao }}" required>
