            ...
            <div class="p-10">
                ...
                <div class="flex flex-col justify-center">
                    <form class="w-full p-6 space-y-6 sm:px-10 sm:space-y-8 bg-green-50 border border-green-500"
                        method="post" action="{{ route('outro.update', ['outro' => $outro]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="flex flex-wrap">
                            ...
                            <input id="data" type="date" class="focus:ring focus:ring-green-200 form-input w-full" name="data"
                                value="{{ $outro->data }}" required>
                            {{ $errors->has('data') ? $errors->first('data') : '' }}

                        </div>

                        <div class="flex flex-wrap">
                            <label for="outros" class="block text-black text-sm font-bold mb-2 sm:mb-4">
                                {{ __('Outros') }}:
                            </label>

                            <input id="outros" type="text" class="focus:ring focus:ring-green-200 form-input w-full" name="outros"
                                value="{{ $outro->outros }}" required>
                            {{ $errors->has('outros') ? $errors->first('outros') : '' }}
                        </div>
